...
# Context
We're exploring a system where an agent learns to navigate voice-based interactions — specifically, making outbound "calls" to retrieve information from a CRM. Think of it as a simulated environment where the agent dials into a system, speaks (or receives speech), and learns to extract the right data through trial and error.

# The Problem
Build an environment where an agent can initiate a voice-based call via MCP and attempt to retrieve a specific piece of information from a prepopulated CRM dataset (e.g., a contact's contract renewal date, account status, or last interaction).
...
- A voice/dialogue layer — the agent's actions take the form of natural language utterances (e.g., asking for a record, clarifying a name, confirming details). How you model this layer is up to you.
...
- The CRM data, MCP server, and dialogue layer should be functional enough to run a full episode: the agent receives a task (e.g., "find the contract value for Acme Corp"), takes actions through the voice/dialogue interface, calls MCP tools, and receives a reward.
...
- The environment should handle realistic failure modes — the agent asks for a record that doesn't exist, provides an ambiguous query, or retrieves the wrong field. These should be reflected in the state and reward.
  - Agent getting an answering machine.
  - Agent getting an incorrect phone number.

## Part 2: The Agent Loop
Wire up a basic training or evaluation loop:
- An agent that can run multiple episodes against the environment, with observable behavior across runs.
- The formulation doesn't need to be sophisticated, but it should be a real loop — not just a single hardcoded trajectory. 
- We should be able to see the agent's behavior, even if learning is minimal in the time available.
...
- What would it take to swap in a real voice API (e.g., speech-to-text/text-to-speech) in place of the simulated dialogue layer?
