...
export class Logger {
  ...
  episodeSummary(result: EpisodeResult): void {
    ...
    console.log(chalk.bold.white('│') + ` Answer:       ${result.submittedAnswer ?? '(none)'}`.padEnd(58) + chalk.bold.white('│'));
  ...
  runSummary(results: EpisodeResult[]): void {
    ...
    const simpleLookup = results.filter(r => r.task.type === 'SIMPLE_LOOKUP');
    const disambiguation = results.filter(r => r.task.type === 'DISAMBIGUATION');

    const slSuccess = simpleLookup.filter(r => r.success).length;
    const dSuccess = disambiguation.filter(r => r.success).length;
    ...
    console.log(chalk.bold.white('╠' + '═'.repeat(58) + '╣'));
    if (simpleLookup.length > 0) {
      const slRate = ((slSuccess / simpleLookup.length) * 100).toFixed(1);
      const slAvgReward = (simpleLookup.reduce((s, r) => s + r.totalReward, 0) / simpleLookup.length).toFixed(2);
      console.log(chalk.bold.white('║') + ` SIMPLE_LOOKUP:  ${slSuccess}/${simpleLookup.length} (${slRate}%)  avg reward: ${slAvgReward}`.padEnd(58) + chalk.bold.white('║'));
    }
    if (disambiguation.length > 0) {
      const dRate = ((dSuccess / disambiguation.length) * 100).toFixed(1);
      const dAvgReward = (disambiguation.reduce((s, r) => s + r.totalReward, 0) / disambiguation.length).toFixed(2);
      console.log(chalk.bold.white('║') + ` DISAMBIGUATION: ${dSuccess}/${disambiguation.length} (${dRate}%)  avg reward: ${dAvgReward}`.padEnd(58) + chalk.bold.white('║'));
    }
    console.log(chalk.bold.white('╚' + '═'.repeat(58) + '╝'));
  }
}
